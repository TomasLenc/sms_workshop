...
uicontrol( ...
    'Style','text', ...
    'Units','normalized', ...
    'Position',[0.01 0.18 0.20 0.04], ...
    'BackgroundColor','white', ...
    'String','z=zoom   p=pan   e=edit');
...
function waveformClickFcn(~,~)
    ...
    % [~, idx] = min(sum( ( cp(1,1:2)' - [data.handles.waveformLine.XData; data.handles.waveformLine.YData] ).^2, 1)); 
    % data.tap_onsets(end+1) = data.handles.waveformLine.XData(idx);
...
function waveMarkerButtonFun(~,~)
    data = guidata(f);
    data.showMarkersOnWaveform = ...
        ~data.showMarkersOnWaveform;
    if data.showMarkersOnWaveform
        data.handles.waveMarkerButton.String = ...
            'Markers ON';
    else
        data.handles.waveMarkerButton.String = ...
            'Markers OFF';
